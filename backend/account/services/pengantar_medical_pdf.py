"""
PDF overlay for SURAT PENGANTAR MEDICAL CHECK UP CPMI.

Place the scanned blank form at **one** of:

    backend/account/assets/pengantar_medical_template.jpg
    backend/account/assets/pengantar_medical_template.jpeg
    backend/account/assets/pengantar_medical_template.png

The template is drawn **letterboxed** inside A4 (see ``pengantar_referral_layout``).
If your medical scan differs from the psychology form, tune ``pengantar_referral_layout``
or set DEBUG_GRID and adjust shared row constants there.
"""
...
def resolve_medical_referral_template_path() -> str:
    for name in (
        "pengantar_medical_template.jpg",
        "pengantar_medical_template.jpeg",
        "pengantar_medical_template.png",
    ):
...
def generate_pengantar_medical_pdf(profile) -> bytes:
    """
    Same field layout as surat pengantar psikologi; tanggal tes from ``tgl_medical``.
    """
    ...
    tpl = resolve_medical_referral_template_path()
    if tpl:
        ...
        c.drawString(
            ...
            "pengantar_medical_template.jpg (atau .jpeg / .png)",
        )
        c.setFillColorRGB(0, 0, 0)
    ...
    tanggal_tes = _fmt_date(getattr(profile, "tgl_medical", None))
