...
## KMS-Connect Backend - Digital Ocean (1 vCPU, 2GB RAM)

This guide will walk you through deploying the KMS-Connect Backend to a Digital Ocean droplet step by step.
...
- Domain name configured (e.g., `data.kms-connect.com`)
...
cd ~
git clone https://github.com/your-username/kms-connect.git
...
DEBUG=0
ALLOWED_HOSTS=data.kms-connect.com,your-server-ip
CSRF_TRUSTED_ORIGINS=https://data.kms-connect.com
...
curl -I http://data.kms-connect.com/health/

# Test HTTPS (after SSL setup)
curl https://data.kms-connect.com/health/
```
...
│   ├── data.kms-connect.com.conf          # SSL config
│   ├── data.kms-connect.com.http-only.conf # HTTP-only config (default)
...
- **HTTP:** http://data.kms-connect.com (redirects to HTTPS after SSL)
- **HTTPS:** https://data.kms-connect.com
