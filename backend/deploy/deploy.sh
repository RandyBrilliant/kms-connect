...
# Deploys the KMS-Connect Backend to production
...
if [ ! -f "$APP_DIR/nginx/data.kms-connect.com.http-only.conf" ]; then
    echo -e "${RED}Error: HTTP-only nginx config not found!${NC}"
    exit 1
fi

# Check if SSL certificates exist
if [ -f "$APP_DIR/nginx/ssl/data.kms-connect.com/fullchain.pem" ] && \
   [ -f "$APP_DIR/nginx/ssl/data.kms-connect.com/privkey.pem" ] && \
   [ -f "$APP_DIR/nginx/ssl/data.kms-connect.com/chain.pem" ]; then
    echo "  ✓ SSL certificates found, using SSL configuration"
    # Verify SSL config file exists
    if [ ! -f "$APP_DIR/nginx/data.kms-connect.com.conf" ]; then
    ...
    if ! grep -q "./nginx/data.kms-connect.com.http-only.conf:/etc/nginx/conf.d/data.kms-connect.com.conf:ro" docker-compose.prod.yml || \
       grep -q "# - ./nginx/data.kms-connect.com.http-only.conf" docker-compose.prod.yml; then
